...
Функция СсылкаСтрокой(Ссылка, УказыватьТипЯвно)
	...
	Возврат Вычислить(ШаблонВыгрузкиСсылки(Ссылка.Метаданные(), УказыватьТипЯвно));
...
Функция ШаблонВыгрузкиСсылки_Справочник(МетаданныеСсылки)
	...
	Если МетаданныеСсылки.ДлинаКода > 0 Тогда
		ШаблонВыгрузкиСсылки = "Ссылка.Код";
	ИначеЕсли МетаданныеСсылки.ДлинаНаименования > 0 Тогда
		//ШаблонВыгрузкиСсылки = """[Н]""+Ссылка.Наименование";
		ШаблонВыгрузкиСсылки = "Ссылка.Наименование";
	КонецЕсли;
	
	ШаблонВыгрузкиСсылки=ШаблонВыгрузкиСсылки+"+?(Ссылка.ЭтоГруппа,""[Г]"","""")";
	
	Если МетаданныеСсылки.СерииКодов = Метаданные.СвойстваОбъектов.СерииКодовСправочника.ВПределахПодчинения Тогда
		ШаблонВыгрузкиСсылки = ШаблонВыгрузкиСсылки + "+?(Ссылка.Родитель.Пустая(), """",""[Р]{""+СсылкаСтрокой(Ссылка.Родитель, Истина)+""}"")";
	КонецЕсли;
	...
	Если Буфер.Владельцы.Количество() > 0 Тогда
		ШаблонВыгрузкиСсылки = ШаблонВыгрузкиСсылки + "+""[В]{""+СсылкаСтрокой(Ссылка.Владелец, Истина)+""}""";
	КонецЕсли;
...
Функция ШаблонВыгрузкиСсылки_Документ(МетаданныеСсылки)
	Возврат "Ссылка.Номер+""[Д]""+Формат(Ссылка.Дата, ""ДФ=yyyyMMdd"")";
КонецФункции
