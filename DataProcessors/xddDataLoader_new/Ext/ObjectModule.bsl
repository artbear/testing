...
Функция СоздатьДанныеПоТабличномуДокументу(Макет) Экспорт
	...
	мОчищенныеКоллекции = Неопределено;
...
Функция ПрочитатьТаблицуЗначенийИзОбластиМакета(Область)
	...
	Для счСтр = 3 по Область.ВысотаТаблицы Цикл
		...
		Для счКол = 1 по ТаблицаИзМакета.Колонки.Количество() Цикл
			...
			Иначе
				Значение = НайтиСоздатьСсылкуПоСтрокеИлиСтруктуре(Значение, мдТип);
			КонецЕсли;
...
КонецФункции
Функция НайтиСоздатьСсылкуПоСтрокеИлиСтруктуре(СтрокаИлиСтруктура, ТипМетаданных) Экспорт
