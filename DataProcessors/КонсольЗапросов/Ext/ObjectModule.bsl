...
Функция СохранитьЗапросВФайл(Запрос, Путь) Экспорт   
	...
	НоваяСтрока.ПараметрыЗапроса = Новый ТаблицаЗначений;
...
Функция СохранитьЗапросыИзСКДВФайл(ОтчетНаСКД, Путь) Экспорт
	...
	//Для каждого НаборДанных из ОтчетНаСКД.СхемаКомпоновкиДанных.НаборыДанных Цикл
	//	
	//	НоваяСтрока = Дерево.Строки.Добавить();
	...
	//			НоваяСтрокаВложенная = НоваяСтрока.Строки.Добавить();
	//			НоваяСтрокаВложенная.Запрос = ВложенныйНаборДанных.Имя;
	//			НоваяСтрокаВложенная.СпособВыгрузки = 1;
	//			НоваяСтрокаВложенная.ПараметрыЗапроса = тПараметры.Скопировать();
	//			НоваяСтрокаВложенная.ТекстЗапроса = ВложенныйНаборДанных.Запрос;
	//		КонецЦикла;
	//	Иначе
...
Функция СмотретьВТ(ЗапросИлиМВТ, ИмяВТ) Экспорт
	Запрос = Новый Запрос;
	Если ТипЗнч(ЗапросИлиМВТ) = Тип("Запрос") Тогда
		Запрос.МенеджерВременныхТаблиц = ЗапросИлиМВТ.МенеджерВременныхТаблиц;
	Иначе
		Запрос.МенеджерВременныхТаблиц = ЗапросИлиМВТ;
	КонецЕсли;
