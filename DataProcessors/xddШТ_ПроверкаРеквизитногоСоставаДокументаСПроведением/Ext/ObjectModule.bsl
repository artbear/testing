...
#Область _// ЭКСПОРТНЫЕ МЕТОДЫ

Процедура Инициализировать(Декоратор) Экспорт
	
	мДекоратор = Декоратор;
	
КонецПроцедуры

Процедура ВыполнитьПроверку(Макет) Экспорт
	ПроведениеДокументов_ЛинейноеЗаполнениеРегистра(Макет);
КонецПроцедуры
...
Процедура ПроведениеДокументов_ЛинейноеЗаполнениеРегистра(Макет) Экспорт
	...
	ТестовыеДанные = мДекоратор.СоздатьДанныеПоТабличномуДокументу(Макет);
	
	ПараметрыТеста = ТестовыеДанные.ПараметрыТеста;
	ВидДокумента = ПараметрыТеста.ВидДокумента;
	ИмяРегистра = ПараметрыТеста.ИмяРегистра;
	ИмяТаблицыДвжиений = ПараметрыТеста.ИмяТаблицыДвжиений;
	...
	ТаблицаДвижений = ТестовыеДанные.ТаблицаДвижений;
	...
	ПроведениеДокументов_ЛинейноеЗаполнениеРегистра_ПоВидуДокумента_МетаданныеЗаполненыПолностью(ТаблицаДвижений);
	
		;;Расшифровка = ПроведениеДокументов_ЛинейноеЗаполнениеРегистра_ПоВидуДокумента_ОшибкиЗаполнения(ТаблицаДвижений);
		;;мДекоратор.ОтладочнаяИнформация(Расшифровка, "Ошибка заполнения таблицы "+ИмяТаблицыДвжиений+" по документу " + ВидДокумента);
	
	ТабличныйДокумент = ПолучитьТабличныйДокументДляГенерацииДанных(ТаблицаДвижений, МетаданныеДокумента, МетаданныеРегистра);
		;;мДекоратор.ОтладочнаяИнформация(ТабличныйДокумент, "Табличный документ для генерации данных таблицы "+ИмяТаблицыДвжиений+" по документу " + ВидДокумента);
		
	ОжидаемыйРезультат = мДекоратор.СоздатьДанныеПоТабличномуДокументу(ТабличныйДокумент).ОжидаемыйРезультат;
	
	ДокументОбъект = ОжидаемыйРезультат[0].Регистратор.ПолучитьОбъект();
	ДокументОбъект.ВыполнитьВКонтексте("ВыполнитьДвиженияПоРегистру_" + ИмяРегистра + "()");
	ФактическийРезультат = ДокументОбъект.Движения[ИмяРегистра].Выгрузить();
	
	мДекоратор.ПроверитьРавенствоТаблиц(ОжидаемыйРезультат, ФактическийРезультат, "Не совпали движения по регистру " + ИмяРегистра + " у документа " + ВидДокумента + " для таблицы " + ИмяТаблицыДвжиений);
...
Функция ПроведениеДокументов_ЛинейноеЗаполнениеРегистра_ПоВидуДокумента_ОшибкиЗаполнения(ТаблицаДвижений)
	
	ТабличныйДокумент = мДекоратор.ПолучитьТабличныйДокументПоТаблицеЗначений(ТаблицаДвижений);
...
Процедура ДополнитьТаблицуДвиженийЗначениями(ТаблицаДвижений, МетаданныеДокумента, МетаданныеРегистра)
	...
	Для каждого Строка из НайденныеСтроки Цикл
		...
		ИначеЕсли нрег(Строка.ТипЗначения) = "булево" Тогда
			Строка.ЗначениеЗаполнения = Истина;
	...
	Для каждого Строка из ТабличныеЧасти Цикл
		...
		ИначеЕсли ПустаяСтрока(Строка.ИмяТабличнойЧасти) Тогда
			Продолжить;
		КонецЕсли;
		НоваяСтрока = ТаблицаДвижений.Вставить(0);
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаСсылка);
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
	КонецЦикла;
	
КонецПроцедуры
Функция ПолучитьТабличныйДокументДляГенерацииДанных(ТаблицаДвижений, МетаданныеДокумента, МетаданныеРегистра)
	...
	Для каждого Строка из ТаблицыБД Цикл
		...
		ТабличныйДокументПоОдномуДокументу = мДекоратор.ПолучитьТабличныйДокументПоТаблицеЗначений(ТаблицаПоОдномуДокументу);
